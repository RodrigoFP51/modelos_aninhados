...
A idéia aqui é estimar modelos de regressão linear para cada continente, de modo que possamos obter coeficientes para cada um deles em particular. No entanto, ao invés de utilizar somente um modelo de regressão para cada continente, irei estimar diversos modelos através de um método chamado bootstrap.    
...
O objetivo principal é poder estimar coeficientes e intervalos de confiança mais roubustos.
...
df_models %>% 
  ...
  labs(x='', y='') + 
  theme(panel.grid.major.x = element_line(size = 0.2, linetype = 2, color = "#74bd6c"))
```
